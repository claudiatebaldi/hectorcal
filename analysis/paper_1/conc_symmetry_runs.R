...
# Hector calibration paper 1.
models_to_fit <- c('CMCC-CESM', 'GFDL-CM3', 'MRI-CGCM3')
...
TEMP_OUTPUT_DIR <- file.path(BASE_DIR, 'output', 'conc_S_kappa_temp')
...
cmip_individual %>%
    filter(variable == 'tas' & model %in% c("CMCC-CESM", "GFDL-CM3", "MRI-CGCM3")) %>%
...
# temperature dynamics of the comparison data.
mapply(function(data_name, comp_data){
    ...
    lapply(kappa_values, function(kappa){
        ...
        # Save output
        file_name <- paste0('conc_', kappa, '_', data_name, '.rds')
...
}, data_name = names(temp_comparison_data), comp_data = temp_comparison_data)
...
TEMPHF_OUTPUT_DIR <- file.path(BASE_DIR, 'output', 'conc_S_kappa_tempHF')
dir.create(TEMPHF_OUTPUT_DIR)

# Subset the comparison data so that it only contains the data for the models
# we are interested in plotting.
cmip_individual %>%
    filter(model %in% c("CMCC-CESM", "GFDL-CM3", "MRI-CGCM3")) %>%
...
mapply(function(data_name, comp_data){
    ...
    lapply(kappa_values, function(kappa){
        ...
        rslt <- tryCatch({
            # Use optim to minimize the MSE between Hector and ESM output data
            stats::optim(par = best_guess, fn = fn, control = list('maxit' = 800))
