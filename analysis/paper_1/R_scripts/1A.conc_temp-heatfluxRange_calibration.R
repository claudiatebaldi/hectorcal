...
## Concentration driven single ESM calibration using temperature and the cmip ESM heat flux range
## as comparison data.
...
# Set up dirs
BASE_DIR   <- getwd()
OUTPUT_DIR  <- file.path(BASE_DIR, 'output', '1A.calibration_results', 'conc_temp-heatfluxRange')
...
hectorcal::cmip_individual %>%
  filter(!grepl(pattern = 'esm', experiment) & variable %in% c('tas')) %>%
...
# Split up the esm data sets up by model, the relevant heatflux cmip range will be added inside of the
# mapply in the find best fits section.
temp_subset %>%
    filter(model %in% c( "GFDL-ESM2G", 'GISS-E2-H-CC', 'GISS-E2-R-CC')) %>%
...
mapply(function(data_name, comp_data){
  ...
  if(!file.exists(file.path(OUTPUT_DIR, paste0('conc_', data_name, '.rds')))){
    ...
    #best_guess <- generate_inital_guess(comp_data, fit_params)
      best_guess <- c(5.105607, 3.005515, 2.103254, 1.180947)
      names(best_guess) <- c( 'S',     'diff',    'alpha',   'volscl' )

    # Subset the heatflux data so that it only contains information for the
    # future experiment we have results for but not histoircal heat flux
    # because those values look too high.
    esm_comparison %>%
      filter(variable == 'heatflux' & experiment %in% comp_data$experiment) %>%
      filter(!grepl(x = tolower(experiment), pattern = 'historical')) %>%
      mutate(sig = 0.15) %>%
    ...
    rslt <- singleESM_calibration_diag(inifiles = ini_files_tib$ini_file,
                                       ...
                                       cmip_range = heatflux_range_data,
                                       n_parallel = 1,
                                       maxit = 800,
                                       showMessages = TRUE)
