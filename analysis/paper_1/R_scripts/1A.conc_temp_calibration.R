...
# Set up dirs
BASE_DIR   <- getwd()
OUTPUT_DIR  <- file.path(BASE_DIR, 'output', '1A.calibration_results', 'conc_temp')
...
mapply(function(data_name, comp_data){
  ...
  if(!file.exists(file.path(OUTPUT_DIR, paste0('conc_', data_name, '.rds')))){
      ...
      best_guess <- c( 0.6487032, 884.1006718, 1.1815411,  2.4436706 )
...
}, data_name = names(esm_data_list)[[1]], comp_data = esm_data_list[[1]])
