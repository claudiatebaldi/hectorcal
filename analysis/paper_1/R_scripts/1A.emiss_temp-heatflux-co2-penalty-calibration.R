...
# Set up dirs
BASE_DIR   <- getwd()
OUTPUT_DIR <-  file.path(BASE_DIR, 'output', '1A.calibration_results', 'emiss_temp-heatflux-co2-penalty')
...
mapply(function(data_name, comp_data){
    ...
    object     <- readRDS(list.files(file.path(BASE_DIR, 'output', '1A.calibration_results', 'emiss_temp-heatflux-co2', 'final'), data_name, full.names = TRUE))
    ...
    saveRDS(rslt, file = file.path(OUTPUT_DIR, paste0('emiss_', data_name, '.rds')))
