...
## okay what I think might acutally make a better summary stat is the temperature RMSE, not including the heat flux
##
...
fits %>%
    filter(comp_data == 'temp') %>%
    filter(min_value == min(min_value) | min_value == max(min_value) | min_value == median(min_value)) ->
    extreme_temp_fits
...
lapply(X = split(extreme_min_max_temp_fits, extreme_min_max_temp_fits$model), FUN = function(input = X){
...
# The other idea is to look at the temperature MSE values that were returned as part of the
# diagnostic calibration results output.
...
## 2. Select Extreeme Fits -----------------------------------------------------------------------------------------------------
# Select and visualize the fits with the smallest, median, and largest fits.
fits %>%
    filter(min_value == min(min_value) | min_value == max(min_value) | min_value == median(min_value)) ->
    extreme_min_max_temp_fits

# Create a tibble of the inputs.
tibble(ini = c(system.file('input/hector_rcp26_constrained.ini', package = 'hector'),
               system.file('input/hector_rcp45_constrained.ini', package = 'hector'),
               system.file('input/hector_rcp60_constrained.ini', package = 'hector'),
               system.file('input/hector_rcp85_constrained.ini', package = 'hector')),
...
# Initiate the hector cores with the ini files.
core_list <- mapply(FUN = newcore, inifile = hector_ini_tib$ini, name = hector_ini_tib$name)

# For each of the
lapply(X = split(extreme_min_max_temp_fits, extreme_min_max_temp_fits$model),
       FUN = function(input = X){

           params <- as.numeric(input[3:6])
           names(params) <- c(ECS(), DIFFUSIVITY(), AERO_SCALE(), VOLCANIC_SCALE())
...
ggplot(data = hector_temp_output) +
    geom_line(aes(year, value, color = model, group = interaction(experiment, model)))
