...
## Calibrate 6/7 Hector parameters at fixed values of beta. This script uses the parameter penalty!
...
OUTPUT_DIR    <- file.path(BASE_DIR, 'output', 'emiss_beta_q10_penaltyBetaPrior')          # where to save the restults to
BESTGUESS_DIR <- file.path(BASE_DIR, 'output', 'emiss_temp_heatflux_co2_penalty', 'calibrationBeta') # where other emission calibration results are saved, these  fits will inform  the initial guesses for the optim routine.dir.create(OUTPUT_DIR)
dir.create(OUTPUT_DIR)

# Define the models with the comparison data we would like to fit to,
models_to_fit <- c("CESM1-BGC", "GFDL-ESM2G", "CanESM2")

# The sampled valeus of beta.
q10_values   <- seq(from = 0.01, to = 8, length.out = 10)
...
mapply(function(data_name, comp_data){
  ...
  best_guess <- object$optim_rslt$par[names(object$optim_rslt$par) != 'q10_rh']
  ...
  lapply(q10_values, function(q10){

      file_name <- paste0('emiss_', data_name, '_', signif(q10, 2), '.rds')

      if(!file.exists(file.path(OUTPUT_DIR, file_name))){


    # Set the beta value.
    lapply(core_list, parameterize_core, params = c('q10_rh' = q10))
    ...
    rslt           <- stats::optim(par = best_guess, fn = fn, control = list('maxit' = 1500))
    rslt[['q10_rh']] <- q10
