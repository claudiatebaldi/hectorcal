...
rcps <- c('26','45','60','85')
conc_files <- file.path(indir, paste0('concen-rcp',rcps, '.rds'))
...
names(hector_conc_ensemble) <- c(paste0('rcp', c('26','45','60','85')),
                                 'params')
use_data(hector_conc_ensemble, overwrite=TRUE, compress='xz')

emiss_files <- file.path(indir, paste0('emissCC-esmrcp', rcps, '.rds'))
...
names(hector_emiss_ensemble) <- c(paste0('rcp', c('26','45','60','85')),
                                  'params')
use_data(hector_emiss_ensemble, overwrite=TRUE, compress='xz')

### I don't think we are actually going to use these, and having lots of package
### data really slows down the package load.  The input files are in the input
### directory under data-raw if we ever change our minds.
...
pc_conc <- readRDS(file.path(indir, 'pc-conc-rcp26-rcp45-rcp60-rcp85.rds'))
use_data(pc_conc, overwrite=TRUE, compress='xz')

pc_emiss <- readRDS(file.path(indir, 'pc-emissCC-esmrcp85.rds'))
