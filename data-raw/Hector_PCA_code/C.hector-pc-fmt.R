...
compute_pc <- function(scenlist, experiments, variables, years)
{
    ...
    alldata <- dplyr::filter(alldata, variable %in% variables, year %in% years)
    ...
    varchk <- dplyr::group_by(alldata, experiment) %>%
      summarise(complete=all(variables %in% variable))
    assert_that(all(varchk$complete))

    yrchk <- dplyr::group_by(alldata, experiment, variable) %>%
      summarise(complete=all(years %in% year))
    ...
    ## Add our metadata
    pca$meta_data <- list(year=sort(years), variable=sort(variables),
...
conc_files <- list.files('PCA-data', '^concen-rcp', full.names=TRUE)
conc_scenlist <- lapply(conc_files, readRDS)
conc_expts <- sapply(conc_scenlist, function(d){d$experiment[1]})
## We want to do all combinations of these scenarios.  There are 15 possible
...
conc_pcas <- foreach(i=1:15) %dopar%
{
    expts_l <- as.logical(c(bitwAnd(i,1), bitwAnd(i,2), bitwAnd(i,4), bitwAnd(i,8)))
    expts <- conc_expts[expts_l]
    scenlist <- conc_scenlist[expts_l]

    compute_pc(scenlist, expts, 'tas', 1851:2100)
}
...
## For the emissions run we really only want the rcp85 run
emiss_files <- 'PCA-data/emissCC-esmrcp85.rds'
emiss_scenlist <- lapply(emiss_files, readRDS)
emiss_pca <- compute_pc(emiss_scenlist, 'esmrcp85', c('tas','co2'), 1851:2100)
saveRDS(emiss_pca, file.path(outdir, 'pc-emissCC-esmrcp85.rds'))
