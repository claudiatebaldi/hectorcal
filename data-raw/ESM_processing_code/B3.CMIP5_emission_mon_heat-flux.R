## B3.CMIP5_emission_mon_heat-flux.R
...
setwd('/pic/projects/GCAM/Dorheim/hectorcal/data-raw/ESM_processing_code')
OUTPUT <- file.path(getwd(), 'output'); dir.create(OUTPUT)
# 1. Define Functions -----------------------------------------------------------------------
...
calculate_global_vars <- function(input){
    ...
    }, error = function(e){input_info %>%  mutate(problem_file = TRUE)})
...
read.csv('output/cmip5_flux_to-process.csv', stringsAsFactors = FALSE) %>%
    rename(variable_file_path = file) %>%
    select(variable_file_path, domain, model, experiment, ensemble, variable, date = time) ->
    cmip_data_files

read.csv('output/cmip5_meta-data_to-process.csv', stringsAsFactors = FALSE) %>%
...
meta_data_files %>%
    ...
    filter(!grepl('HadGEM2-ES_historical_r1i1p1', meta_file_path)) %>%
...
cmip_data_files %>%
    inner_join(oceanWeights_tibble, by = 'model') %>%
...
# Save the output
write.csv(x = global_var_results, file = file.path(OUTPUT, 'CMIP5_emission_global_monthly_flux.csv'), row.names = FALSE)
